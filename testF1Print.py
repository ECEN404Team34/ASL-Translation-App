...
classifier.compile(
              ...
              metrics = ['accuracy', tf.keras.metrics.Precision(), tf.keras.metrics.Recall(), f1])   #add f1 score
...
'''#Saving the model
import h5py
classifier.save('Trained_sec_model.h5')'''
...
axs[0,0].plot(model.history['val_accuracy'], label="Valildation Accuracy")
...
axs[1,1].plot(model.history['f1'], label="Training F1")
axs[1,1].plot(model.history['val_f1'], label="Validation F1")
axs[1,1].set_title('Training and Validation F1 Score')
...
current_model = load_model('Trained_sec_model.h5')
